...
function mean(town, strng) {
  ...
  let counter = 0;
  strng.map((x,i)=>{
    if(x.split(':')[0]!=town) return
    x.replace(`${town}:`,'').split(',').map((x,i)=>{
      console.log(i)
      counter+=+x.split(' ')[1]
    })
...
function variance(town, strng) {
  ...
  strng.map((x,i)=>{
    if(x.split(':')[0]!=town) return
    x.replace(`${town}:`,'').split(',').map(x=>{
     	console.log(Math.pow(Math.abs(+x.split(' ')[1]- avg), 2))
      counter+=(Math.pow(Math.abs(+x.split(' ')[1]- avg), 2))
    })
